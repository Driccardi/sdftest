 ...
 * @NModuleScope Public
 */
define(["N/search", "N/runtime", "N/format", "N/error", "N/record"], /**
 * @param {search} search
 ...
 */ function (search, runtime, format, error, record) {
    ...
    NSUtil.catchNull = (strVal) => {
        strVal = strVal == null ? "" : strVal;
        return strVal;
    };

    NSUtil.catchIsNaN = (flAmount, intPrecision = 2) => {
        return !isFinite(parseFloat(flAmount)) ? 0 : parseFloat(flAmount).toFixed(intPrecision);
    };

    NSUtil.parseNumber = (flAmount, intPrecision = 2) => {
        return !isFinite(parseFloat(flAmount)) ? 0 : parseFloat(flAmount);
    };

    NSUtil.formatDate = (objParam) => {
        const dtNewDate = new Date(objParam.date); // or any other date you want to format
        const objOptions = objParam.options;
        const strFormattedDate = new Intl.DateTimeFormat(objParam.locale, objOptions).format(dtNewDate);
    ...
    NSUtil.removeDuplicate = function (arrValue) {
        if (
            arrValue === "" || //Strict checking for this part to properly evaluate integer value.
            arrValue == null ||
            arrValue == undefined
        ) {
            return arrValue;
    ...
    NSUtil.getPostingPeriodByDate = function (stDate) {
        ...
        var arrFilters = [search.createFilter({ name: "startdate", operator: search.Operator.ONORBEFORE, values: stDate }), search.createFilter({ name: "enddate", operator: search.Operator.ONORAFTER, values: stDate }), search.createFilter({ name: "isquarter", operator: search.Operator.IS, values: "F" }), search.createFilter({ name: "isyear", operator: search.Operator.IS, values: "F" }), search.createFilter({ name: "isadjust", operator: search.Operator.IS, values: "F" })];

        var arrColumns = [search.createColumn({ name: "startdate", sort: search.Sort.ASC }), search.createColumn({ name: "enddate" })];
    ...
    NSUtil.isClosedDatePostingPeriod = function (stDate) {
        ...
        var objPdSearch = search.create({
            ...
            filters: [["startdate", "onorbefore", stDate], "AND", ["enddate", "onorafter", stDate], "AND", ["isyear", "is", "F"], "AND", ["isquarter", "is", "F"], "AND", ["closed", "is", "F"], "AND", ["alllocked", "is", "F"]],
            columns: ["periodname"],
        });
    ...
    NSUtil.isClosedPostingPeriod = function (stPeriodName) {
        ...
        var objPdSearch = search.create({
            type: "accountingperiod",
            filters: [["periodname", "is", stPeriodName], "AND", ["isyear", "is", "F"], "AND", ["isquarter", "is", "F"], "AND", ["closed", "is", "F"], "AND", ["alllocked", "is", "F"]],
            columns: ["periodname"],
        });
            ...
            var _copyDeployment = function (scriptId) {
                try {
                    var objSrch = Search.create({
                        ...
                        filters: [["script.scriptid", "is", scriptId], "AND", ["status", "is", "NOTSCHEDULED"], "AND", ["isdeployed", "is", "T"]],
                        columns: ["scriptid"],
                    });
    ...
    NSUtil.lookupFields = function (options) {
        ...
        Object.keys(resultObj).forEach(function (key) {
            if (util.isArray(resultObj[key]) && resultObj[key].length > 0 && resultObj[key][0].hasOwnProperty("value")) {
