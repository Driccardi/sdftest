 ...
 */ (serverWidget, log, runtime, task, redirect) => {
    ...
    const onRequest = (scriptContext) => {
        ...
        if (scriptContext.request.method == "GET") {
            ...
            const objParam = scriptContext.request.parameters;
            const objScript = runtime.getCurrentScript();
            /*const strBuildWoMapReduce = objScript.getParameter({
                name: "custscript_ns_build_wo_map_reduce",
            });
            */
            const strBuildWoMapReduce = "ns_mr_build_wo";
            //-------------------------------------POST METHOD-------------------------------------
            // Call a map/reduce script
            var mapReduceTask = task.create({
                taskType: task.TaskType.MAP_REDUCE,
                scriptId: "customscript_" + strBuildWoMapReduce,
                deploymentId: "customdeploy_" + strBuildWoMapReduce,
                params: {
                    custscript_ns_prod_plus_rec: objParam.custpage_transaction_id,
                },
            });
